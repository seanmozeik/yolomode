...
import { ADJECTIVES, ANIMALS, IMAGE } from './constants';
...
export async function confirm(question: string): Promise<boolean> {
  ...
  return new Promise((resolve) => {
    rl.question(`${pc.yellow(pc.bold('confirm:'))} ${question} [y/N] `, (answer) => {
      rl.close();
...
export async function ensureRunning(id: string): Promise<void> {
...
export async function resolveSession(
  ...
  die(`${sessions.length} sessions — specify a name:\n${sessions.map((n) => `  ${n}`).join('\n')}`);
}
...
// Spawn a docker exec shell (nu) with proper TTY size forwarding.
export async function execShell(id: string, workDir: string): Promise<void> {
...
export async function copyImports(
  ...
  for (const { abs, base } of imports) {
    ...
    await run($`tar -cf - -C ${dir} ${base} | docker exec -i ${id} tar -xf - -C /tmp/imports/`);
  }
