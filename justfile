...
# Always re-fetches Claude Code + Codex + Pi Agent + RTK; use --no-cache for a full rebuild
docker-build *args:
  GITHUB_TOKEN=$(gh auth token) docker build --secret id=gh_token,env=GITHUB_TOKEN --no-cache-filter=claude-install,codex-install,pi-install,tool-rtk {{args}} .
